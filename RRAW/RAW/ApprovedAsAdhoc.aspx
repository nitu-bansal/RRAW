...
<body onload="try{top.document.getElementById('processing_image').style.display='none'}catch(e){};javascript:document.getElementById('txtHAWBNumber').focus();function f(){var t=new Date();return((t.getMonth()+1)+'/'+t.getDate()+'/'+t.getFullYear()+' '+t.getHours()+':'+t.getMinutes()+':'+t.getSeconds())};document.getElementById('hidCurrentDateTime').value=f();">
    ...
    <div style="position: fixed; text-align: center; text-decoration: underline; width: 100%;">
        <div class="page_title">
           Old Approved As Adhoc Rates</div>
    </div>
    <br />
    <br />
            ...
            <tr>
                <td id="tdOperations" colspan="32" align="left" style="height: 40px;">
                    <div id="divOperations" style="position: fixed; margin-top: -18px">
                        <asp:Button ID="btnFilter" runat="server" OnClientClick="DisplayProgress();" Text="Apply Filter" />&nbsp;
                        <asp:Button ID="btnRemoveFilter" runat="server" OnClientClick="DisplayProgress(); ClearFields();"
                            Text="Remove Filter" />&nbsp;
                        <asp:Button ID="btnExportToCSV" runat="server" EnableViewState="False" Visible="False"
                            Text="Export To CSV" />&nbsp;
                        <asp:Button ID="btnExportToExcel" runat="server" EnableViewState="False" Visible="False"
                            Text="Export To Excel" />&nbsp;
                        <asp:Button ID="btnRemoveRequest" runat="server" EnableViewState="False" Visible="False"
                            Enabled="false" OnClientClick="DisplayProgress();" Text="Remove" />&nbsp;
                        <asp:Button ID="btnTransferToTariff" runat="server" EnableViewState="False" Visible="False"
                            Enabled="false" OnClientClick="DisplayProgress();" Text="Transfer To Tariff" />&nbsp;
                        <asp:Button ID="btnGenerateNewRateRequest" runat="server" EnableViewState="False"
                            Visible="False" Enabled="false" OnClientClick="DisplayProgress();" Text="Generate New Rate Request" />&nbsp;
                        <asp:Button ID="btnConvertCurrency" runat="server" OnClientClick="DisplayProgress();" Text="Convert Currency" Visible="false" />
                        <br />
                    </div>
                </td>
            </tr>
